...
|---|---|---|
| **Google ADK 2.x** (primary) | `GOOGLE_API_KEY` or `GEMINI_API_KEY` | An ADK `LlmAgent` on Gemini (`ATLAS_ADK_MODEL`, default `gemini-3.5-flash-lite`), grounded in the engine through function tools (`atlas/agents/router.py`); it commits its reading via a tool call, and the deterministic graph still decides who is accountable. |
